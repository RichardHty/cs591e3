...
public class OnFlingListener implements View.OnTouchListener {
    ...
    private final class GestureListener extends GestureDetector.SimpleOnGestureListener {
        ...
        public boolean onFling(MotionEvent e1, MotionEvent e2, float velocityX, float velocityY) {
            if (e1 != null && e2 != null) {
                if (e1.getX() - e2.getX() > Constants.FLING_MIN_DISTANCE && Math.abs(velocityX) > Constants.FLING_THRESHOLD_VELOCITY) {
                    onRightToLeft();
                    return true;
                } else if (e2.getX() - e1.getX() > Constants.FLING_MIN_DISTANCE && Math.abs(velocityX) > Constants.FLING_THRESHOLD_VELOCITY) {
                    onLeftToRight();
                    return true;
                }

                if (e1.getY() - e2.getY() > Constants.FLING_MIN_DISTANCE && Math.abs(velocityY) > Constants.FLING_THRESHOLD_VELOCITY) {
                    onBottomToTop();
                    return true;
                } else if (e2.getY() - e1.getY() > Constants.FLING_MIN_DISTANCE && Math.abs(velocityY) > Constants.FLING_THRESHOLD_VELOCITY) {
                    onTopToBottom();
                    return true;
                }
            }
